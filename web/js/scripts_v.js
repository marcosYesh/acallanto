...
var _cAcallanto_Scripts = {
	...
	fnBanners: function(){
		bnsURLs = ['aAcallanto.html','instCredenciadas.html','sejaAcallanto.html','nossasUnidades.html','aAcallanto.html'];
		...
		var getBanner = function(){
			...
			jQuery('section.banners > ul li').eq(currBns-1).fadeIn(fadeTransition,function(){
				jQuery('section.banners aside > a').attr('href',bnsURLs[currBns-1]);
	...
	fnNavMenu : function(){
		...
		if(menu.length > 0){
			menu.click(function(e){
				if( jQuery(this).find('a').attr('target') )return;
				e.preventDefault();
